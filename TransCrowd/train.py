...
def main(args):
    ...
    elif args['dataset'] == 'Pig_detection':
        train_file = './npydata/Pig_detection_train.npy'
        test_file = './npydata/Pig_detection_test.npy'
    ...
    if args['pre']:
        if os.path.isfile(args['pre']):
            ...
            try:
                ...
                # For loading pretrained DeiT weights that don't have state_dict structure
                model.load_state_dict(checkpoint, strict=False)
                args['start_epoch'] = 0
                args['best_pred'] = float('inf')
        else:
...
def train(Pre_data, model, criterion, optimizer, epoch, args, scheduler):
    ...
    metrics_file_path = os.path.join(args['save_path'], 'train_metrics.txt')
    with open(metrics_file_path, 'a') as f:
        f.write(f"Epoch: {epoch}, Loss: {losses.avg:.4f}, Batch Time: {batch_time.avg:.3f}, Data Time: {data_time.avg:.3f}, LR: {args['lr']:.10f}\n")
...
def validate(Pre_data, model, args):
    ...
    # Save eval metrics to file
    metrics_file_path = os.path.join(args['save_path'], 'eval_metrics.txt')
    with open(metrics_file_path, 'a') as f:
        f.write(f"Valid Result  MAE: {mae:.3f}, MSE: {mse:.3f}\n")
