...
class FCRN_A(nn.Module):
    ...
    def __init__(self, N: int=1, input_filters: int=3, **kwargs):
        ...
        # Initialize all layers with proper weights and biases
    ...
    def forward(self, input: torch.Tensor):
        """Forward pass."""
        return torch.relu(self.model(input))
...
class UNet(nn.Module):
    ...
    def __init__(self, filters: int=64, input_filters: int=3,**kwargs):
        ...
        self.density_pred = nn.Conv2d(in_channels=filters, out_channels=1,
                                    kernel_size=(1, 1), bias=True)  # Add bias
        
        # Initialize the final layer with small positive values
        nn.init.kaiming_normal_(self.density_pred.weight, mode='fan_out', nonlinearity='relu')
        nn.init.constant_(self.density_pred.bias, 0.1)  # Small positive bias
    ...
    def forward(self, input: torch.Tensor):
        ...
        return torch.relu(self.density_pred(block7))
